...
class VAEModule(pl.LightningModule):
    """
    A PyTorch Lightning Module for training a Variational Autoencoder (VAE).

    This module encapsulates the model architecture, loss function, training,
    and validation logic for the VAE stage of the pipeline. It is configured
    via an OmegaConf object.
    """
    ...
    def get_last_layer(self):
        """Returns the weights of the last convolutional layer of the decoder."""
        return self.torch_nn_module.decoder.conv_out.weight

    def configure_optimizers(self):
        """Sets up the optimizers and learning rate schedulers."""
    ...
    def training_step(self, batch: torch.Tensor, batch_idx: int):
        """Performs a single training step with manual optimization."""
        ...
        aeloss, log_dict_ae = self.loss(
            frames, reconstructions, posterior,
            optimizer_idx=0, global_step=self.trainer.global_step, split="train",
        ...
        discloss, log_dict_disc = self.loss(
            frames, reconstructions, posterior,
            optimizer_idx=1, global_step=self.trainer.global_step, split="train",
            last_layer=self.get_last_layer()
        )
    ...
    def validation_step(self, batch: torch.Tensor, batch_idx: int):
        ...
        aeloss, log_dict_ae = self.loss(
            frames, reconstructions, posterior,
            optimizer_idx=0, global_step=self.trainer.global_step, split="val",
            last_layer=self.get_last_layer()
        )
        ...
        if batch_idx == 0:
            ...
            visualize_sequence(
                save_path=save_path,
                sequences=[vis_frames, vis_recons],
                labels=["Ground Truth", "VAE Reconstruction"],
