...
if __name__ == '__main__':
    # Specify the paths for the original and new checkpoints
    old_checkpoint_path = "pretrained/vae/pretrained_imerg_vae_8x8x64_v1.pt"
    new_checkpoint_path = "pretrained/vae/pretrained_imerg_vae_8x8x64_v2.pt"

    # Load the state dict from the old checkpoint (this also applies any necessary legacy migrations)
    state_dict = pl_ckpt_to_state_dict(old_checkpoint_path)

    # Save the new checkpoint containing the updated state dict
    torch.save({"state_dict": state_dict}, new_checkpoint_path)
    print("Converted checkpoint saved to", new_checkpoint_path)