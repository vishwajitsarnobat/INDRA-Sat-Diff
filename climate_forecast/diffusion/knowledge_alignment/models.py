...
class ContiguousGroupNorm(nn.GroupNorm):
    def forward(self, x):
        # Enforce standard contiguous memory layout
        if not x.is_contiguous(memory_format=torch.contiguous_format):
            x = x.contiguous(memory_format=torch.contiguous_format)
        return super().forward(x)

nn.GroupNorm = ContiguousGroupNorm
    ...
    def __init__(self,
        ...
        if pool == "adaptive":
            self.out = nn.Sequential(
                ContiguousGroupNorm(min(cuboid_out_channels, 32), cuboid_out_channels),
                nn.SiLU(),
    ...
    def forward(self, x, t, verbose=False, **kwargs):
        ...
        if self.readout_seq:
            ...
            if self.num_global_vectors > 0:
                ...
                out_global = out_global.contiguous(memory_format=torch.contiguous_format)
